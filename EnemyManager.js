...
export default class EnemyManager {
    ...
    this.enemyTypes = [
      {
        name: "Mask Dude",
        sprite: new Sprite(
          `${basePath}/Mask Dude/Run (32x32).png`,
          32,
          32,
          12,
          0.05,
      ...
      },
      {
        name: "Pink Man",
        sprite: new Sprite(
          `${basePath}/Pink Man/Run (32x32).png`,
          32,
          32,
          12,
          0.05,
      ...
      },
      {
        name: "Ninja Frog",
        sprite: new Sprite(
          `${basePath}/Ninja Frog/Idle (32x32).png`,
          32,
          32,
          11,
          0.05,
  ...
  update(deltaTime, player, platforms, effects) {
    ...
    for (let i = this.enemies.length - 1; i >= 0; i--) {
      ...
      const facingPlayer =
        (playerCenterX < enemyCenterX && enemy.vx <= 0) ||
        (playerCenterX > enemyCenterX && enemy.vx >= 0);
      const playerVisible =
        inSight &&
        sameYLevel &&
        (facingPlayer ||
          horizontalDistance <= this.sightRange * 0.45 ||
          player.vy < -80);
      ...
      if (enemy.state === "chase") {
        ...
        const boxLeft =
          enemy.vx < 0 ? enemy.x - lookAheadDist : enemy.x + enemy.width;
        const boxRight =
          enemy.vx < 0 ? enemy.x : enemy.x + enemy.width + lookAheadDist;
        ...
        const terrainAhead = platforms?.some((plat) => {
          ...
          const horizOverlap =
            boxRight > plat.x && boxLeft < plat.x + plat.width;
          const vertOverlap =
            eyeY > plat.y - 2 && eyeY < plat.y + plat.height + 2;
        ...
        const stillVisible =
          horizontalDistance <= this.sightRange * 1.1 &&
          verticalDistance <= this.chaseYTolerance &&
          (playerCenterX < enemyCenterX
            ? enemy.vx <= 0
            : enemy.vx >= 0 || horizontalDistance <= this.sightRange * 0.45);
        if (stillVisible) {
          enemy.chaseSightLostTimer = this.chaseGraceTime;
        } else {
      ...
      if (platforms) {
        for (let plat of platforms) {
          ...
          // Overlap check
          const horizOverlap =
            enemy.x < plat.x + plat.width && enemy.x + enemy.width > plat.x;
          // Allow walking on top of terrain if enemy.y + enemy.height is at or near plat.y
          const vertOverlap =
            enemy.y + enemy.height > plat.y + 4 &&
            enemy.y < plat.y + (plat.height || 40);
      ...
      const overEnemyGap = this.gaps.some(
        (gap) => enemyCenterXPos > gap.x && enemyCenterXPos < gap.x + gap.width,
      );
      ...
      if (platforms && enemy.vy >= 0) {
        for (let plat of platforms) {
          ...
          const verticalOverlap =
            prevBottom <= plat.y + 6 && currentBottom >= plat.y - 2;
      ...
      if (enemy.grounded && enemy.state !== "chase") {
        const lookAhead =
          enemy.vx > 0 ? enemy.x + enemy.width + 8 : enemy.x - 8;
        const nearGapEdge = this.gaps.some(
          (gap) => lookAhead > gap.x && lookAhead < gap.x + gap.width,
        );
        if (nearGapEdge) {
          enemy.vx *= -1;
        }
    ...
    for (let e of enemyData) {
      ...
      for (let gap of this.gaps) {
        if (spawnX + 16 > gap.x && spawnX < gap.x + gap.width) {
          ...
          spawnX =
            distToLeft < distToRight ? gap.x - 40 : gap.x + gap.width + 8;
        }
      }
